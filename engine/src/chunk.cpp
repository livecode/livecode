...
Exec_stat MCChunk::set(MCExecPoint &ep, Preposition_type ptype)
{
    ...
    if (destvar != nil)
    {
        ...
        if (istextchunk())
        {
            ep . copyasmutablestringref(t_string);
            settextchunk(ep, (MCStringRef)*t_value, ptype, t_string);
            MCEngineExecPutIntoVariable(ctxt, t_string, PT_INTO, destvar);
        }
        else
            MCEngineExecPutIntoVariable(ctxt, *t_value, ptype, destvar);
    }
    else if (url != nil)
    {
        ...
        if (istextchunk())
        {
            ...
            settextchunk(ep, (MCStringRef)*t_value, ptype, t_string);
            MCNetworkExecSetUrl(ctxt, *t_url, t_string);
        }
        else
            MCNetworkExecPutIntoUrl(ctxt, (MCStringRef)*t_value, ptype, *t_url);
    }
    else
    {
		if (getobj(ep, t_object, True) != ES_NORMAL)
        ...
        if (istextchunk())
        {
            ...
            settextchunk(ep, (MCStringRef)*t_value, ptype, t_string);
            t_object . object -> setstringprop(ctxt, t_object . part_id, P_TEXT, False, t_string);
        }
        else
