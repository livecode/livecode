...
{
	MCSystemSendMail(ctxt, p_to, p_cc, p_subject, p_body);
}
...
{	
	MCSystemPrepareMail(ctxt, p_to, p_cc, p_bcc, p_subject, p_body, p_type);
	...
	if (p_attachments != nil)
	{
		if (MCArrayIsSequence(p_attachments))
		{
			...
			for(uindex_t i = 0; i < MCArrayGetCount(p_attachments); i++)
			{
				...
				MCSystemAddAttachment(ctxt, t_data, t_file, t_type, t_name);
			}	
		}
		else
		{
	...
	MCSystemSendPreparedMail(ctxt);
}

void MCMailGetCanSendMail(MCExecContext& ctxt, bool& r_result)
{
	MCSystemGetCanSendMail(ctxt, r_result);
}
