...
/* WRAPPER */ bool MCCrypt_rsa_op(bool p_encrypt, bool p_is_public, MCStringRef p_message_in, MCStringRef p_key, MCStringRef p_passphrase, MCStringRef &r_message_out, MCStringRef &r_result, uint32_t &r_error)
{
	...
	char *t_result;
    char *t_message_in, *t_key, *t_passphrase;
	...
	bool t_success = MCCrypt_rsa_op(p_encrypt, p_is_public ? RSAKEY_PUBKEY : RSAKEY_PRIVKEY, t_message_in, MCStringGetLength(p_message_in),
									t_key, MCStringGetLength(p_key), p_passphrase != nil ? t_passphrase : nil,
							  ...
							  MCStringRef p_salt, MCStringRef p_iv, uint2 p_bit_rate, MCStringRef &r_output, MCStringRef &r_result, uint32_t &r_error)
{
	...
	t_ssl_encode = SSL_encode(p_is_decrypt, MCNameGetCString(p_cipher), t_data, MCStringGetLength(p_data),
								t_outlen, t_key, MCStringGetLength(p_key), p_is_password, p_bit_rate,
								p_salt != nil ? t_salt : nil, p_salt != nil ? MCStringGetLength(p_salt) : 0,
								p_iv != nil ? t_iv : nil, p_iv != nil ? MCStringGetLength(p_iv) : 0);
