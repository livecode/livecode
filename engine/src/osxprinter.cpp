...
MCPrinterDialogResult MCMacOSXPrinter::DoDialog(bool p_window_modal, Window p_owner, bool p_is_settings)
{
	...
	if (!MCModeMakeLocalWindows())
	{
		...
		if (t_success)
		{
            MCAutoStringRef t_data_str;
            /* UNCHECKED */ MCStringCreateWithCString(t_data, &t_data_str);
			if (p_is_settings)
				MCRemotePrintSetupDialog(&t_reply_data, t_reply_data_size, t_result, *t_data_str, t_data_size);
			else
				MCRemotePageSetupDialog(&t_reply_data, t_reply_data_size, t_result, *t_data_str, t_data_size);
			t_success = deserialize_printer_settings(MCStringGetCString(*t_reply_data), t_reply_data_size, m_session, t_printer, m_settings, m_page_format);
		}
		if (t_success)
		{
