...
static bool unpickle_object_from_stream(IO_handle p_stream, MCStringRef p_version, MCStack *p_stack, MCObject*& r_object)
{
	bool t_success;
	t_success = true;
...
MCObject *MCObject::unpickle(MCDataRef p_data, MCStack *p_stack)
{
	...
	while(t_length > 0 && t_success)
	{
		...
		t_object = nil;
		if (t_success)
			t_success = unpickle_object_from_stream(t_stream, t_5500_only ? MCSTR("5.5") : MCSTR("2.7"), p_stack, t_object);
		...
		if (t_success && !t_5500_only)
		{
			...
			if (unpickle_object_from_stream(t_stream, MCSTR("5.5"), p_stack, t_other_object))
			{
				delete t_object;
