...
MCRectangle MCPlayer::getcontrollerpartrect(const MCRectangle& p_rect, int p_part)
{
    switch(p_part)
    {
        ...
        case kMCPlayerControllerPartThumb:
        {
            ...
            float t_current_time, t_duration;
            t_current_time = (float)getmoviecurtime();
        ...
        {
            float t_finish_time, t_duration;
            t_finish_time = (float)getendtime();
            t_duration = (float)getduration();
        ...
        {
            float t_start_time, t_finish_time, t_duration;
            t_start_time = (float)getstarttime();
            t_finish_time = (float)getendtime();
            t_duration = (float)getduration();
        ...
        {
            float t_start_time, t_current_time, t_finish_time, t_duration;
            t_duration = (float)getduration();
...
void MCPlayer::handle_mdown(int p_which)
{
    ...
    switch(t_part)
    {
        ...
        case kMCPlayerControllerPartWell:
        {
            ...
            t_new_time = (mx - t_part_well_rect . x) * (float(t_duration) / t_part_well_rect . width);
    ...
    if (state & CS_MFOCUSED)
    {
        switch(m_grabbed_part)
        {
            ...
            case kMCPlayerControllerPartThumb:
            {
                ...
                // PM-2014-08-22 [[ Bug 13257 ]] Make sure t_new_time will not overflow
                t_new_time = (x - t_part_well_rect . x) * (float(t_duration) / t_part_well_rect . width);
            ...
            case kMCPlayerControllerPartSelectionStart:
            {
                ...
                t_new_start_time = (x - t_part_well_rect . x) * (float(t_duration) / t_part_well_rect . width);
            ...
            case kMCPlayerControllerPartSelectionFinish:
            {
                ...
                t_new_finish_time = (x - t_part_well_rect . x) * (float(t_duration) / t_part_well_rect . width);
