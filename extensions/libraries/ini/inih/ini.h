...
LIBRARY_DLLEXPORT_DEF typedef int (*ini_handler)(void* user, const char* section,
...
LIBRARY_DLLEXPORT_DEF typedef char* (*ini_reader)(char* str, int num, void* stream);
...
*/
LIBRARY_DLLEXPORT_DEF int ini_parse(const char* filename, ini_handler handler, void* user);

/* Same as ini_parse(), but takes a FILE* instead of filename. This doesn't
   close the file when it's finished -- the caller must do that. */
LIBRARY_DLLEXPORT_DEF int ini_parse_file(FILE* file, ini_handler handler, void* user);

/* Same as ini_parse(), but takes an ini_reader function pointer instead of
   filename. Used for implementing custom or string-based I/O (see also
   ini_parse_string). */
LIBRARY_DLLEXPORT_DEF int ini_parse_stream(ini_reader reader, void* stream, ini_handler handler,
                     void* user);

/* Same as ini_parse(), but takes a zero-terminated string with the INI data
instead of a file. Useful for parsing INI data from a network socket or
already in memory. */
LIBRARY_DLLEXPORT_DEF int ini_parse_string(const char* string, ini_handler handler, void* user);
