...
def usage(exit_status):
    print(
"""Create single archive of prebuilt library packages for the platform.

Usage:
  upload.py [--target-platform PLATFORM] [--target-arch ARCH] [--target-subplatform SUBPLATFORM]

Options:
  -p, --target-platform TARGET
                    Choose which target platform to build archive for
  -a, --target-arch ARCH
                    Choose which target arch to build archive for
  -s, --target-subplatform SUBPLATFORM
                    Choose which target subplatform to build archive for
...
def exec_archive_prebuilts(build_platform, build_arch, build_subplatform):
	# set curdir to prebuilt folder
	os.chdir(os.path.dirname(__file__))
	if platform.system() == 'Windows':
		args = ["..\util\invoke-unix.bat", "./archive-libs.sh", build_platform, build_arch, build_subplatform]
	else:
		args = ["./archive-libs.sh", build_platform, build_arch, build_subplatform]
	print(' '.join(args))
...
def archive(args):
    ...
    print('Creating target platform prebuilt library archive (' + opts['TARGET_PLATFORM'] + ',' + opts['TARGET_ARCH'] + ',' + opts['TARGET_SUBPLATFORM'] + ')')
    exec_archive_prebuilts(opts['TARGET_PLATFORM'], opts['TARGET_ARCH'], opts['TARGET_SUBPLATFORM'])
